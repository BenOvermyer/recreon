...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), all four world/fleet command units (CONSTR, MSCCOMM, FLTCOMM, CLSCOMM), and the in-game menu bar. **FLTCOMM also closed Phase 4's §4.3.** What remains is **the interactive half of ATTCOMM (§5.3)** and the §8.3 status windows (NMSWIND, HLPWIND). Every other menu command is wired.
