...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), and all four world/fleet command units — CONSTR.PAS, MSCCOMM.PAS, FLTCOMM.PAS and CLSCOMM.PAS. **FLTCOMM also closed Phase 4's §4.3.** What remains is ATTCOMM (§5.3, the interactive combat UI) and MENU.PAS's pull-downs to hang the lot on.
