...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), all four world/fleet command units (CONSTR, MSCCOMM, FLTCOMM, CLSCOMM), the naming commands (NAMES.PAS), and the in-game menu bar. **FLTCOMM closed Phase 4's §4.3 and ATTCOMM's interactive half closed Phase 5's §5.3.** What remains is the §8.3 status windows (NMSWIND, HLPWIND). **Every menu command is wired** — `ui/menu.py`'s `PENDING` is down to `AboutCom`, whose whole implementation is a message.
