...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), the construction commands (CONSTR.PAS), and the defense/self-destruct commands (MSCCOMM.PAS). What remains of it is the rest of §8.2 — MENU.PAS's pull-downs and the `*COMM.PAS` handlers (CLSCOMM, FLTCOMM, ATTCOMM) — and the status windows (§8.3), plus the leftover UIs from Phases 4–6.
...
Thirty-seven are catalogued so far (#7–#10, #12–#18, #20–#21, #23–#30, #33–#35, #37–#40, #46–#50, #54–#55). #54 and #55 came out of PROLOG.PAS: a manual save that destroys the autosave through the file it borrows as scratch, and `AddPlayerEmpire` discarding technologies it has already gathered. #46–#50 came out of LOADSAVE.PAS and MESS.PAS: a `LoadGame` that never cleans up the universe it is replacing, and four in the message subsystem. #11 was withdrawn — it described a porting slip in `set_status`, not an original defect. Read them before "fixing" anything that looks broken in a ported module: it is probably already known and deliberate.
