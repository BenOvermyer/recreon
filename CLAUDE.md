...
**`ScoutFleets` and `ScoutObjects` are still unported** (Phase 8, #5 §8.1), so probes are an NPE's only way to learn anything — which is why `update_probes` is wired into the turn loop ahead of them. Until they land the AI is fighting half-blind: every decision gated on `Known()` or `Scouted()` sees less than it should, and `get_best_target` in particular skips worlds the empire has not scouted.
