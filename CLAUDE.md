...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), and the construction commands (CONSTR.PAS). What remains of it is the rest of §8.2 — MENU.PAS's pull-downs and the `*COMM.PAS` handlers (CLSCOMM, FLTCOMM, MSCCOMM, ATTCOMM) — and the status windows (§8.3), plus the leftover UIs from Phases 4–6.
