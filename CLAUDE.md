...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, and the scenario front end (§8.5). What remains of it is the menus (§8.2) and the status windows (§8.3), plus the leftover UIs from Phases 4–6.

**A game now starts from the UI.** `recreon` with no `--scenario` opens the picker (`ui/newgame.py`): choose a scenario, page the introduction, pick a player count, name the empires, galaxy builds. `--scenario` still skips straight in.
...
Thirty-five are catalogued so far (#7–#10, #12–#18, #20–#21, #23–#30, #33–#35, #37–#40, #46–#50). #46–#50 came out of LOADSAVE.PAS and MESS.PAS: a `LoadGame` that never cleans up the universe it is replacing, and four in the message subsystem. #11 was withdrawn — it described a porting slip in `set_status`, not an original defect. Read them before "fixing" anything that looks broken in a ported module: it is probably already known and deliberate.
