...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), all four world/fleet command units (CONSTR, MSCCOMM, FLTCOMM, CLSCOMM), and the in-game menu bar. **FLTCOMM also closed Phase 4's §4.3.** What remains is **the interactive half of ATTCOMM (§5.3)** plus the handful of commands listed in `ui/menu.py`'s `PENDING` that have no screen yet.

**ATTCOMM.PAS is half ported.** `AutoAttackCommand` and `GetTarget` are done (`attcomm.py`, `ui/attack.py`), so a player can fight from the menu. `AttackCommand` — the round-by-round loop where the player moves groups between orbital shells, retargets and retreats — is not: `GetGroups`, `WarpIn`, `WarpOut`, `GroupTarget`, `GroupRetreat`, `GroupMove` and `Engage` remain. The mechanics they drive are already in `attack.py`/`attnpe.py`; what is missing is the player's hand on them.
