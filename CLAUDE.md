...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), and the prologue menu (PROLOG.PAS). What remains of it is the in-game menus (§8.2, MENU.PAS and the `*COMM.PAS` handlers) and the status windows (§8.3), plus the leftover UIs from Phases 4–6.
