...
Phase 8 so far: the scouting sweep, the map buffer, `GetNewsLine`, save/load (§8.4) with the message subsystem it depends on, the scenario front end (§8.5), the prologue menu (PROLOG.PAS), and three of the four command units — CONSTR.PAS, MSCCOMM.PAS and FLTCOMM.PAS. **FLTCOMM also closes Phase 4's §4.3**, so fleets can now be deployed, transferred, refuelled and given orders from the UI. What remains is CLSCOMM (the world close-up and production screens, which is really §8.3 status-window work), ATTCOMM (§5.3, the interactive combat UI), and MENU.PAS's pull-downs to hang the lot on.

**Command screens are bound directly to keys until MENU.PAS lands**: `f` fleets, `b` construction, `w` warp links, `d` defenses, `x` self-destruct, `g` the prologue menu.
...
Thirty-nine are catalogued so far (#7–#10, #12–#18, #20–#21, #23–#30, #33–#35, #37–#40, #46–#50, #54–#55, #59–#60). #54 and #55 came out of PROLOG.PAS; #59 and #60 out of FLTCOMM.PAS — editing a fleet's orders can leave its resume point past the end of the list (a crash in the port, so clamped), and mine sweeping costs 100 starships as a compiled order but nothing as an interactive command. #46–#50 came out of LOADSAVE.PAS and MESS.PAS: a `LoadGame` that never cleans up the universe it is replacing, and four in the message subsystem. #11 was withdrawn — it described a porting slip in `set_status`, not an original defect. Read them before "fixing" anything that looks broken in a ported module: it is probably already known and deliberate.
