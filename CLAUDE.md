...
**`DisplayBackground` is unported, and it is a real gap.** `EnemyConquered` calls SCENA.PAS's `DisplayBackground(..., Conquer := True)` before its generic victory speech, and a scenario with a `WORLDBACKGROUNDINDEX` entry for that world replaces the speech entirely — **7 of the 13 shipped scenarios have one**. CLSCOMM.PAS's close-up screen calls it too and `clscomm.py` skips it the same way. The subsystem is SCENA.PAS's (`FindLine`, `SplitLine`, `IDMatch`, `SatisfiesConditions`, `DisplayText`), not ATTCOMM's, and belongs with the rest of the scenario front end. Until it lands, every conquest gets the generic wording.
