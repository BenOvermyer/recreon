...
**Current state: Phases 1–6 complete except their UIs (§4.3, §5.3, CONSTR.PAS); Phase 7 (AI) started.** Ported: `types.py` (TYPES.PAS), `datastrc.py` (DATASTRC.PAS), `datacnst.py` (DATACNST.PAS), `cdetypes.py` (CDETYPES.PAS), `npe/types.py` (NPETYPES.PAS), `galaxy.py` (GALAXY.PAS), `environ.py` (ENVIRON.PAS + `InitializeUniverse` from LOADSAVE.PAS), `misc.py` (MISC.PAS), `primintr.py` and `intrface.py` (PRIMINTR/INTRFACE.PAS — the subsets used so far, now including the whole name subsystem), `news.py` (NEWS.PAS), `update.py` (UPDATE.PAS world update), `design.py` (DESIGN.PAS designation + INTRFACE's `DesignateWorld`/`TerraformWorld`), `resource.py` (RESOURCE.PAS), `newgame.py` (NEWGAME.PAS), `orders.py` (ORDERS.PAS), `fleet.py` (FLEET.PAS), `attack.py` (ATTACK.PAS), `attnpe.py` (ATTNPE.PAS), `battle.py` (BATTLE.PAS), `sbase.py` (SBASE.PAS), `npe/core.py` (NPEINTR.PAS), `npe/common.py` (NPE00.PAS), `utils/` (INT.PAS, REAL1.PAS, DFA.PAS), the turn loop in `main.py`, and `ui/`.

**`npe/core.py` and `npe/common.py` are ported but nothing calls them yet.** NPEINTR.PAS is the primitives layer and NPE00.PAS is the shared behaviour built on it — between them, everything an AI does except decide the order to do it in. What is still missing is the deciding: the four personas (NPE01–NPE04) and the NPE.PAS dispatcher. Until those land, `update_turn` still skips an NPE's turn.

`npe/dispatch.py` (NPE.PAS) is deliberately **not** written yet. It is a five-way `CASE` over the personas and cannot be exercised without them, so writing it early would leave a module that imports files that do not exist — worse than an absent one under the "an absent file means unported" rule.

This remains the window to fix the AI's arithmetic before anything depends on its output. The list has grown: **#23, #28 and #29** distort what the AI believes, and **#34** means it will not reinforce a world that has no ships — the freshly conquered and freshly raided ones. **#35** is a hang rather than a wrong number and is the strongest candidate for an early exception to the port-stays-faithful rule.
...
**`conquer_empire` iterates `SetOfPlanetsOf`, which the original only rebuilds at load and at `CreatePlanet`** — never on `SetStatus`. Within a session the set therefore goes stale: worlds the empire has lost are still swept, worlds it has taken are not. The DOS build reloaded between turns (hot-seat/PBEM), which hid it. Ported literally; do not "fix" it without deciding that's a deviation you want.
...
Twenty-five are catalogued so far (#7–#18, #20–#21, #23–#30, #33–#35). Read them before "fixing" anything that looks broken in a ported module: it is probably already known and deliberate.
